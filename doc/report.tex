...
\title{\textbf{The Li-Chao tree: An Alternative Approach to Dynamic Maintenance of the Upper Envelope}}
\author{Li Chao}
\date{\today}
...
\begin{abstract}
The Li-Chao tree (LICT) was informally introduced in a lecture in 2012 and subsequently gained widespread adoption in the competitive programming community for dynamic maintenance of the upper (or lower) envelope of a set of lines. Despite its practical utility, the structure has not been formally documented in the academic literature. This technical report formalizes the LICT and provides a comprehensive analysis comparing it with existing solutions including the Dynamic Convex Hull Trick (CHT). The LICT offers distinct advantages in implementation simplicity, numerical stability, and extensibility to advanced variants such as persistence and line segments.
\end{abstract}

\section{Introduction}

Dynamic Line Container maintenance is a fundamental problem in computational geometry with extensive applications in dynamic programming optimization, operations research, and computer graphics. This technical report focuses on the Li-Chao tree (LICT), a data structure that maintains a set of linear functions while supporting insertion and query operations. The problem is defined as follows: given a dynamic set of linear functions $y = kx + b$, support efficient insertion of new lines and querying the minimum (or equivalently, maximum) value at arbitrary $x$ coordinates. This report focuses on minimum queries; maximum queries are obtained by negating line parameters.
...
Both operations must execute in logarithmic time for acceptable performance at scale. The LICT provides $O(\log C)$ time per operation for minimum (or maximum) queries, where $C$ denotes the range of possible $x$-coordinates.
...
\begin{align*}
    L &= \mathbb{I}[f_\text{new}(l) < f_\text{stored}(l)] \\
    M &= \mathbb{I}[f_\text{new}(m) < f_\text{stored}(m)]
\end{align*}
...
\begin{equation*}
    ...
    \end{cases}
\end{equation*}
This follows from the single-intersection property: if the ordering changes between $l$ and $m$, the intersection point lies in $[l, m]$; otherwise, it lies in $[m+1, r]$.
