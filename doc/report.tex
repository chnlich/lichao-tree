...
Figure~\ref{fig:insertion-example} illustrates the insertion of a new line $f_{new}(x) = 0.5x+5$ into a structure containing $y=x$ at $[0,8]$, $y=2x$ at $[0,4]$, and $f_{[4,8]}(x) = 2x-2$ at $[4,8]$.
...
\node[draw, rectangle, fill=red!10, minimum width=2.2cm] (r1) at (3.2,-2.0) {$[4, 8]$: $y=0.5x+5$};
...
\node[draw, rectangle, fill=orange!20, minimum width=1.4cm] (l2rl) at (1.2,-4.5) {$[4, 6]: 2x-2$};
\node[draw, rectangle, minimum width=1.4cm] (l2rr) at (5.2,-4.5) {$[6, 8]: -$};

% Edges
\draw[->, thick] (root) -- (l1);
\draw[->, thick, red] (root) -- (r1) node[midway, above right, red, font=\tiny] {route $f_{new}$};
\draw[->] (l1) -- (l2ll);
\draw[->] (l1) -- (l2lr);
\draw[->, thick, dashed, orange] (r1) -- (l2rl) node[midway, left, orange, font=\tiny, xshift=-2pt] {displaced};
\draw[->] (r1) -- (l2rr);
...
$f_{[4,8]}(6)=10$, $f_{new}(6)=8$.\\
$f_{new}(6) < f_{[4,8]}(6) \Rightarrow$ Swap!\\
Store $f_{new}$; route $f_{[4,8]}$ left.
};
...
Store displaced line $2x-2$.\\
Recursion ends.
};
\end{scope}

\end{tikzpicture}
\caption{Line Insertion Tree View. Inserting $f_{new}(x)=0.5x+5$. At $[4,8]$, $f_{new}$ wins at midpoint ($8<10$) but loses at left endpoint ($7>6$), so it swaps in and the displaced line is routed to $[4,6]$.}
...
\node[draw, rectangle, minimum width=1.4cm] (l2lr) at (-1.2,-4.5) {$[2, 4]: -$};
\node[draw, rectangle, minimum width=1.4cm] (l2rl) at (1.2,-4.5) {$[4, 6]: 2x-2$};
\node[draw, rectangle, minimum width=1.4cm] (l2rr) at (5.2,-4.5) {$[6, 8]: -$};
