...
\begin{abstract}
The Li-Chao Tree (LICT), introduced at ZJOI 2012, was originally designed for online maintenance of the lower envelope of linear functions---a fundamental problem in computational geometry. Over the past decade, it has found widespread application in dynamic programming optimization, far beyond its original scope. This paper presents a retrospective analysis of the LICT alongside the Dynamic Convex Hull Trick (CHT), examining both the theoretical foundations and practical performance characteristics. Through streamlined benchmarks across three scales ($10^5$, $10^6$, and $10^7$ operations), we demonstrate that while the Dynamic CHT maintains a performance advantage of approximately 1.5--1.8$\times$, the LICT's simplicity and extensibility continue to make it a valuable tool in competitive programming and algorithmic problem solving. Our simplified codebase serves as a practical reference for practitioners selecting the appropriate data structure for their needs.
\end{abstract}
...
In the winter of 2012, at the Zhejiang Provincial Olympiad in Informatics (ZJOI), a problem emerged that would lead to a simple yet powerful data structure. The challenge was elegantly stated:
...
This problem, which we call \textbf{online line envelope maintenance}, is deceptively simple. The naive approach---storing all lines and evaluating each at query time---requires $O(n)$ per query. The challenge is to do better.

At the time, the Convex Hull Trick (CHT) was already well-known in competitive programming circles. However, the CHT required either monotonic insertions (for the deque-based variant) or complex intersection point calculations (for the dynamic variant). What was needed was something simpler---a data structure that could handle arbitrary insertions with minimal code, no floating-point arithmetic, and easy correctness proofs.

\subsection{The Insight}

The key insight behind the Li-Chao Tree is remarkably simple: instead of maintaining the convex hull explicitly, maintain the \emph{best line at each interval midpoint}. 

Consider a query range $[x_{\min}, x_{\max}]$. At the midpoint $x_{\text{mid}}$, one line is optimal. This line might also be optimal on some subinterval. We recursively partition the range, storing at each node the line that is currently best at the midpoint. When inserting a new line, we compare it with the stored line at each node: if the new line is better at the midpoint, we swap them and continue with the worse line on the appropriate side. This simple comparison-based approach requires no geometric calculations, no intersection points, no divisions---just integer comparisons of line values.

This insight---that we can maintain the envelope implicitly through divide-and-conquer rather than explicitly through geometry---is the essence of the LICT.

\subsection{Structure of This Paper}

This work is organized as follows. Section~\ref{sec:related} reviews the Convex Hull Trick and its variants, establishing the context in which the LICT was invented. Section~\ref{sec:implementation} presents our streamlined implementations of both the Standard LICT and Dynamic CHT. Section~\ref{sec:benchmarks} provides comprehensive performance benchmarks and analysis. Section~\ref{sec:discussion} discusses the trade-offs between the two approaches and their practical implications. Finally, Section~\ref{sec:conclusion} reflects on the LICT's legacy and enduring relevance.
...
struct LineContainer : multiset<Line, less<>> {
    ...
    // Floored division handling negative numbers correctly
...
The complexity of this implementation lies in the intersection calculations and the careful handling of line removal when a new line makes existing lines obsolete.
...
More than a decade has passed since the Li-Chao Tree was introduced at ZJOI 2012. What began as a solution to a specific contest problem has become a staple of competitive programming and algorithmic problem solving worldwide.

The LICT's enduring popularity is not due to raw performance---the benchmarks confirm that the Dynamic CHT is faster. Rather, it is due to the elegance of the underlying insight: that we can maintain the lower envelope implicitly, through simple comparisons at interval midpoints, without ever computing an intersection point.

This insight---simple in retrospect but not obvious before---is the hallmark of good algorithm design. It reminds us that sometimes the best solution is not the fastest, but the one that balances performance with clarity, extensibility, and reliability.

\subsection{Legacy}

The Li-Chao Tree has outlived its original context. While invented for online line envelope maintenance, it has found its greatest utility in dynamic programming optimization---an application its creator likely did not anticipate. This is the nature of good ideas: they find uses beyond their original purpose.

For practitioners today, the choice between LICT and Dynamic CHT is not about picking a winner, but about understanding the trade-offs. Both are excellent data structures. The LICT offers simplicity and extensibility; the Dynamic CHT offers raw speed. The right choice depends on the constraints of the problem at hand.
