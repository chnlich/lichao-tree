...
The correctness of this invariant follows from Lemma~\ref{lem:linear-dom} and Lemma~\ref{lem:routing-dom} below.
...
The appropriate child is determined by where the relative ordering changes. Let $f_\text{new}(x) = k_\text{new} x + b_\text{new}$ denote the new line and $f_\text{stored}(x) = k_\text{stored} x + b_\text{stored}$ denote the line currently stored at the node. Let $\mathit{lef} = (f_\text{new}(l) < f_\text{stored}(l))$ and $\mathit{midf} = (f_\text{new}(m) < f_\text{stored}(m))$. If $\mathit{lef} \neq \mathit{midf}$, the intersection lies in $[l, m]$, so recurse left; otherwise recurse right. This follows from the single-intersection property: if the ordering changes between $l$ and $m$, the intersection point lies in $[l, m]$; otherwise, it lies in $[m, r]$.
...
\textbf{Remark (tie-breaking).} When $f_\text{new}(m) = f_\text{stored}(m)$, the strict $<$ comparison on line~8 evaluates to false, so the stored line is kept and the new line is pushed down. This convention is arbitrary; since both lines achieve the same value at $m$, either choice satisfies the invariant (Definition~\ref{def:lict}).
