...
\begin{abstract}
The Li-Chao tree (LICT) was informally introduced in a lecture in 2012 and subsequently gained widespread adoption in the competitive programming community as an alternative approach to online line container queries for minimum or maximum values. Despite its practical utility, the structure has not been formally documented in the academic literature. This technical report formalizes the LICT and provides a comprehensive analysis comparing it to the Dynamic Convex Hull Trick (CHT). While the Dynamic CHT has traditionally been the standard solution, the LICT offers distinct advantages in implementation simplicity, numerical stability, and extensibility to advanced variants such as persistence and line segments.
\end{abstract}

\section{Introduction}

Dynamic line container maintenance is a fundamental problem in computational geometry with extensive applications in dynamic programming optimization, operations research, and computer graphics. A dynamic line container is a data structure that maintains a set of linear functions while supporting insertion and query operations. The problem is defined as follows: given a dynamic set of linear functions $y = kx + b$, support efficient insertion of new lines and querying the minimum (or equivalently, maximum) value at arbitrary $x$ coordinates. This report focuses on minimum queries; maximum queries are obtained by negating line parameters.
...
Both operations must execute in logarithmic time to achieve acceptable performance for large-scale applications. The LICT achieves $O(\log C)$ time per operation for minimum (or maximum) queries, where $C$ is the coordinate range.
...
In 2012, the Li-Chao tree was informally presented in a lecture context. It subsequently gained widespread adoption within the competitive programming community. Despite over a decade of practical use and its inclusion in standard algorithm libraries, the structure has not been formally documented in the academic literature. This technical report addresses this gap by providing a formal treatment of the LICT. We present precise algorithmic specifications, implementation details, and empirical performance analysis relative to the established Dynamic CHT baseline. We present well-tested, reference implementations and conduct systematic performance evaluation with particular attention to extensibility features.
...
Each node in the tree represents an interval $[l, r]$ and stores one line. During insertion, a line is \emph{routed} through the tree following a single path from root to leaf. At each node on this path, the new line is compared with the currently stored line. The line that achieves the lower value at the midpoint $m = \lfloor(l+r)/2\rfloor$ is \emph{stored} at that node; the suboptimal line continues downward to be routed through the appropriate child subtree.

\textbf{Key invariant.} A line stored at a node is the best among all lines routed through that node at the midpoint of the node's interval. This is \emph{local} optimality: the stored line minimizes $kx + b$ at midpoint $m$ among lines routed to that specific node. However, this does not guarantee global optimality at $m$ because lines routed down other branches may achieve lower values at that coordinate.

\textbf{Why this maintains the lower envelope.} The LICT stores at each node the line that achieves the lower value at the midpoint among lines routed there. Because two lines intersect at most once, if line $A$ achieves a lower value than line $B$ at the midpoint but yields a higher value at one endpoint, then $B$ can only achieve a lower value on the side of the midpoint containing that endpoint. Thus $B$ is routed to exactly one child---the side where it might achieve a lower value. At query time, we traverse the path to $x_0$ and take the minimum over all stored lines on that path. For any line $L$ and query point $x_0$, consider the unique root-to-leaf path whose intervals all contain $x_0$. Line $L$ is stored at some node on this path if and only if $L$ was routed through that node and achieved the minimum value at its midpoint. Since every line is routed down exactly one path, and the query traverses exactly this path, all lines that could potentially be optimal at $x_0$ are evaluated. The minimum among stored lines on the path therefore equals the global lower envelope at $x_0$.
...
\textbf{Deletion.} Unlike the Dynamic CHT, which can support deletion of arbitrary lines in $O(\log n)$ amortized time, the standard LICT does not support efficient deletion. Removing a line would require traversing all nodes where that line might be stored and recomputing optimal lines from descendants, which requires $\Omega(n)$ time in the worst case. For applications requiring deletion, alternative approaches include segment tree beats or reconstructing the tree periodically.
...
\begin{itemize}
    \item \textbf{Line segments:} Lines valid only on subranges $[x_l, x_r]$ integrate naturally. During insertion, when the current node interval $[l, r]$ lies entirely outside $[x_l, x_r]$, we skip that subtree. When $[l, r]$ lies entirely inside, we proceed normally. Otherwise, we recurse into both children. This limits the line's influence to its valid range without modifying the core algorithm.
...
\newblock China, 2012.\\
\textit{Note: The data structure is conventionally referred to as the ``Li-Chao tree'' in the literature and competitive programming community.}}
