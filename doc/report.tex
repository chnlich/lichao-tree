...
\begin{itemize}
    ...
    \item \textbf{Sparse Cases (Random):} Conversely, the ZKW implementation is \textbf{4x slower} than standard LICT. Our analysis shows that for random inputs, the standard dynamic tree remains extremely sparse, visiting only \textbf{4-5 nodes} on average per query. In contrast, the ZKW implementation's power-of-two padding shifts the tree structure, causing relevant data to be pushed deeper. Even with early exit optimizations, ZKW queries traverse significantly deeper paths (avg depth 24 for $N=10^7$), resulting in higher latency due to cache misses on the large array.
\end{itemize}
