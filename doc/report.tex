...
\begin{proof}
Each full-line insertion follows a single root-to-leaf path. At each existing node, the algorithm performs $O(1)$ comparisons and recurses into exactly one child (Algorithm~\ref{alg:insert}, lines 11--14). When it reaches a null child, it creates exactly one new node and returns (lines 1--3). Therefore each insertion creates at most one new node. With $n$ insertions, the total number of nodes is at most $n$, giving $O(n)$ space.
