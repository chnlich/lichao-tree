...
This section provides a high level overview of both data structures before presenting detailed implementations and empirical analysis.

\subsection{Algorithmic Approach}

The fundamental distinction between the Dynamic CHT and the LICT lies in their approach to envelope maintenance. The Dynamic CHT maintains the convex hull explicitly, tracking intersection points between adjacent lines and removing obsolete lines as new ones are inserted. This explicit geometric approach yields optimal time complexity but requires careful handling of numerical precision.

The LICT, in contrast, maintains the envelope implicitly through a recursive interval subdivision strategy. Each node in the tree represents an interval $[l, r]$ and stores the line optimal at the midpoint. Insertion proceeds by comparing the new line with the stored line at each node: if the new line is better at the midpoint, the lines are swapped and the worse line continues down the appropriate child interval. This approach eliminates intersection calculations entirely, at the cost of exploring the full tree height regardless of hull size.
