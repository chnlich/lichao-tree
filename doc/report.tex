...
Each node in the tree represents an interval $[l, r]$ and stores the line that is locally optimal at the midpoint $m = \lfloor(l+r)/2\rfloor$ among all lines that have been routed to that node during insertion. A line is routed to a node if, during insertion, it was determined to potentially be optimal somewhere within that node's interval. A stored line is locally optimal at $m$ if it yields the minimum value at $m$ compared to all other lines that have been routed to this specific node. Note that this line is not necessarily globally optimal at $m$, as other lines may have been routed to different branches of the tree based on their performance at other midpoints.
