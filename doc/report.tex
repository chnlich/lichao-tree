...
We provide a formal specification of the Li-Chao tree in the Lean 4 theorem prover. The core correctness relies on the geometric property that two lines intersect at most once.

\begin{lstlisting}[language=Haskell, caption=Formal Specification in Lean 4]
...
theorem li_chao_condition (f g : Line) (l m r : Int) 
    (hlm : l < m) (hmr : m < r) :
...
def query (t : Tree) (x : Int) : Int :=
  ...
  | Tree.node l r line left right =>
    let val := line.eval x
    let m := ((l : Int) + (r : Int)) / 2
    if x <= m then min val (query left x)
    else min val (query right x)

-- Correctness Theorem
-- The query returns the minimum value among all inserted lines at x
theorem query_correct (lines : List Line) (x : Int) (l r : Nat) 
    (h_in : l <= x ∧ x <= r) :
  let t_final := lines.foldl insert (build ... l r)
  query t_final x = min (query t_init x) (brute_force_min lines x)
\end{lstlisting}
