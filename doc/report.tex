...
\begin{itemize}
    ...
    \item \textbf{Sparse Cases (Random):} Conversely, the ZKW implementation is \textbf{4x slower} than standard LICT. For random inputs, the dynamic tree remains sparse, allowing many operations to terminate early when encountering null nodes. The ZKW implementation, being fully static, must traverse the full depth ($O(\log C)$) for every query, incurring unnecessary overhead when relevant lines are sparse.
\end{itemize}
