...
Since $L^*$ enters the tree at the root (which lies on $P$) and is not stored on $P$ in the final tree, $L^*$ must have been pushed to a child not containing $x_0$ at some node $w \in P$. At that moment, the line $S$ kept at $w$ satisfies $S(m_w) \leq L^*(m_w)$. We show $S(x_0) \leq L^*(x_0)$:
\begin{itemize}
\item If $L^*$ is pushed left ($L^*(l) < S(l)$, so the intersection lies in $[l, m_w]$): then $x_0 > m_w$, and $S$ dominates $L^*$ on $[m_w, r]$. By Lemma~\ref{lem:linear-dom}, $S(x_0) \leq L^*(x_0)$.
...
If $S$ remains on $P$, then $\text{query}(x_0) \leq S(x_0) \leq L^*(x_0)$, contradicting the assumption. If $S$ is later displaced from $w$ by a new line $C$ (with $C(m_w) < S(m_w)$):
\begin{itemize}
\item If $S$ is routed toward $x_0$: $S$ moves to a deeper node on $P$, and $\text{query}(x_0) \leq S(x_0) \leq L^*(x_0)$, again a contradiction.
\item If $S$ is routed away from $x_0$: by the same argument, $C(x_0) \leq S(x_0) \leq L^*(x_0)$, and $C$ is stored at $w \in P$. If $C$ remains, contradiction. If $C$ is also displaced, repeat.
\end{itemize}
Since the number of displacements is finite, this chain terminates with some line on $P$ achieving $\leq L^*(x_0)$ at $x_0$, contradicting the assumption.
\end{proof}
...
\begin{proof}
Each full-line insertion follows a single root-to-leaf path. At each existing node, the algorithm performs $O(1)$ comparisons and recurses into exactly one child (Algorithm~\ref{alg:insert}, lines 11--14). If the path reaches a null child, it creates one new node and returns (lines 1--3); if the path terminates at an existing leaf, no new node is created. Therefore each insertion creates at most one new node. With $N$ insertions, the total number of nodes is at most $N$, giving $O(N)$ space.
