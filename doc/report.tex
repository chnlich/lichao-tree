...
In the winter of 2012, at the Zhejiang Provincial Olympiad in Informatics (ZJOI)~\cite{zjoi2012}, a deceptively simple problem emerged:
...
To understand the significance of the Li-Chao Tree, we must first examine the landscape of line envelope maintenance techniques that preceded and competed with it. The Convex Hull Trick, in its various forms, was the dominant approach before 2012, and it remains a strong competitor today.
...
Listing~\ref{lst:dcht} shows our Dynamic CHT implementation using \texttt{std::multiset}. This implementation, adapted from KACTL~\cite{kactl}, maintains lines sorted by slope with intersection points computed on insertion.

\begin{lstlisting}[language=C++, caption={Dynamic CHT Implementation (max hull; wrapper class for min)}, label={lst:dcht}]
struct Line {
...
The complexity in this implementation lies in the intersection calculations and the careful handling of line removal when a new line makes existing lines obsolete. Each insertion takes $O(\log N)$ amortized time, where the amortization accounts for the removal of lines that become obsolete due to the new insertion.

In contrast to the Dynamic CHT's geometric approach, the Li-Chao Tree achieves simplicity by avoiding intersection calculations entirely. Instead, it relies on direct value comparisons at interval midpoints.
...
\begin{itemize}
    ...
    \begin{itemize}
        \item \textbf{Random:} Random slopes and intercepts. Expected hull size: $\Theta(\sqrt{N})$ for uniformly random lines (a classic result in computational geometry~\cite{renyi1963}).
...
\textbf{Performance Gap:} The Dynamic CHT is consistently $1.5$--$2.0\times$ faster than the LICT. This gap is most pronounced at smaller scales, where the LICT's pointer-based tree traversal overhead is relatively higher. As $N$ increases, the ratio stabilizes around $1.5\times$.

\textbf{Hull Size Impact:} The relationship between hull size and performance is nuanced. At $N=10^5$, the gap is smaller when all lines are on the hull ($1.63\times$ vs $2.17\times$), suggesting the Dynamic CHT benefits from skipping irrelevant lines in the random case. However, at larger scales ($N \geq 10^6$), this pattern reverses, possibly due to cache effects: when all lines reside on the hull, both structures process more data, and the LICT's pointer-chasing behavior becomes relatively more expensive.
...
These benchmark results quantify the performance trade-off between the two approaches, but numbers alone do not tell the complete story. The practical choice between LICT and Dynamic CHT depends on factors beyond raw speed.
...
\bibitem{renyi1963}
A.~R\'{e}nyi and R.~Sulanke.
\newblock \"{U}ber die konvexe H\''{u}lle von $n$ zuf\''{a}llig gew\''{a}hlten Punkten.
\newblock \textit{Zeitschrift f\''{u}r Wahrscheinlichkeitstheorie und Verwandte Gebiete}, 2(1):75--84, 1963.
