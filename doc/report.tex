...
\textbf{Remark (tie-breaking).} When both lines achieve the same value at $m$, the strict $<$ comparison on line~8 keeps the resident line and pushes the incoming line down. This convention is arbitrary. In particular, inserting a duplicate line (identical $k$ and $b$) triggers this case at every level; the duplicate sinks to maximum depth and is discarded, which is correct since it does not change the lower envelope.
