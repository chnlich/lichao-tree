...
\begin{definition}[LICT Node Structure]\label{def:lict-node}
Each node $v$ in the LICT represents an interval $[l, r]$ and contains:
\begin{enumerate}
\item A stored line $f_v(x) = k_v x + b_v$ (or \texttt{null} if no line has been routed through $v$),
\item Pointers to left and right children, representing subintervals $[l, m]$ and $[m, r]$ respectively, where $m = \lfloor(l+r)/2\rfloor$.
\end{enumerate}
\textbf{Tree invariant:} At each node $v$ with interval $[l, r]$ and midpoint $m$, the stored line $f_v$ achieves the minimum value at $m$ among all lines that have been routed through $v$:
\[
f_v(m) = \min \{ g(m) : g \text{ has been routed through } v \}.
\]
\end{definition}
...
\textbf{Remark (tie-breaking).} When $f_\text{new}(m) = f_\text{stored}(m)$, the strict $<$ comparison on line~8 evaluates to false, so the stored line is kept and the new line is pushed down. This convention is arbitrary but must be consistent; it ensures the tree invariant (Definition~\ref{def:lict-node}) is maintained.
