...
\begin{abstract}
The Li-Chao Tree (LICT) was informally introduced in a lecture in 2012 and subsequently gained widespread adoption in the competitive programming community as an alternative approach to online line container queries. Despite its practical utility, the structure has not been formally documented in the academic literature. This paper, written in 2026, aims to formalize the LICT and provide a comprehensive analysis comparing it to the Dynamic Convex Hull Trick (CHT). While the Dynamic CHT has traditionally been the standard solution, the LICT offers distinct advantages in implementation simplicity, numerical stability, and extensibility to advanced variants such as persistence and line segments.
\end{abstract}
...
Each node in the tree represents an interval $[l, r]$ and stores the line that is optimal at the midpoint $m = \lfloor(l+r)/2\rfloor$ among all lines that have been inserted through that node (i.e., lines whose evaluation at $m$ is better than any line previously stored at this node). Note that this line is not necessarily globally optimal at $m$, as other lines may have been routed to different branches of the tree.
...
Our empirical evaluation (detailed in Section~\ref{sec:benchmarks}) confirms that the Dynamic CHT achieves approximately 1.45--2.00 times faster execution across problem scales ranging from $10^5$ to $10^7$ operations. However, the LICT's implementation requires roughly half the code and eliminates an entire class of potential bugs related to intersection calculations. For applications where implementation time, correctness guarantees, or extensibility are paramount, the LICT offers a compelling alternative despite its higher computational cost.
