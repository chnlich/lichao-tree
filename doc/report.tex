...
\textbf{Key invariant.} A line stored at a node achieves the minimum value among all lines routed through that node at the midpoint of the node's interval. This is \emph{local} optimality: the stored line minimizes $kx + b$ at midpoint $m$ among lines routed to that specific node. However, this does not guarantee global optimality at $m$ because lines routed down other branches may achieve lower values at that coordinate.
