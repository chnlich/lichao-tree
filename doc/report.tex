...
We establish the correctness of the LICT through three results: a foundational property of linear functions, a local invariant maintained by insertion, and a global correctness theorem for queries.
...
\]
the line stored at $v$ achieves the minimum value at $x = m$ among all lines that have been compared at $v$.
\end{lemma}

\begin{proof}
By induction on the number of insertions.

\textbf{Base case.} The first line compared at $v$ is stored directly. The invariant holds trivially.

\textbf{Inductive step.} Suppose the invariant holds before a new line $L_{\text{new}}$ is compared at $v$ against the stored line $L_{\text{stored}}$.
\begin{itemize}
\item If $L_{\text{new}}(m) \geq L_{\text{stored}}(m)$: $L_{\text{stored}}$ remains, still optimal at $m$.
\item If $L_{\text{new}}(m) < L_{\text{stored}}(m)$: $L_{\text{new}}$ replaces $L_{\text{stored}}$, now optimal at $m$ among all lines compared so far.
\end{itemize}
Nodes not on the insertion path are unmodified, so the invariant is preserved there trivially.
\end{proof}
...
\textbf{Claim.} For every inserted line $L$, there exists a node $v \in P$ such that $\text{stored}(v)(x_0) \leq L(x_0)$.

Consider the insertion of $L$. At each node along the insertion path, the winner at the midpoint is stored and the loser (the \emph{pushed line}) is routed to one child.

\textbf{Case 1: $L$ is pushed to the child not containing $x_0$ at some node $w$.}
Let $S$ denote the line stored at $w$ (the winner) and let $L$ be the pushed line (the loser). By construction, $S(m_w) \leq L(m_w)$. We show $S(x_0) \leq L(x_0)$:
\begin{itemize}
\item If $L$ is pushed left ($L(l) < S(l)$, so the intersection lies in $[l, m_w]$): then $x_0 > m_w$. Since $S(m_w) \leq L(m_w)$ and the intersection is in $[l, m_w]$, $S$ dominates $L$ on $[m_w, r]$. By Lemma~\ref{lem:linear-dom}, $S(x_0) \leq L(x_0)$.
\item If $L$ is pushed right ($L(l) \geq S(l)$): then $x_0 \leq m_w$. We have $S(l) \leq L(l)$ and $S(m_w) \leq L(m_w)$. By Lemma~\ref{lem:linear-dom} on $[l, m_w]$, $S(x_0) \leq L(x_0)$.
\end{itemize}

At the moment of routing, $S$ is stored at $w \in P$ and $S(x_0) \leq L(x_0)$.

If $S$ is later displaced from $w$ by a new line $C$ (with $C(m_w) < S(m_w)$):
\begin{itemize}
\item If $S$ is routed toward $x_0$: $S$ moves to a deeper node on $P$, and $S(x_0) \leq L(x_0)$ is still witnessed on $P$.
\item If $S$ is routed away from $x_0$: by the same argument above, $C(x_0) \leq S(x_0) \leq L(x_0)$, and $C$ is now stored at $w \in P$.
\end{itemize}
By induction on the number of subsequent displacements, the claim is maintained.

\textbf{Case 2: $L$ is never pushed off $P$.}
Then $L$ is stored at some node on $P$. If later displaced, the same displacement analysis applies: either $L$ moves deeper along $P$, or the line replacing $L$ dominates it at $x_0$. At a leaf, the winner of the single-point comparison dominates the loser trivially. In all cases, some line on $P$ achieves $\leq L(x_0)$.

Since the claim holds for every inserted line $L$, we have $\text{query}(x_0) \leq \min_i L_i(x_0)$. Combined with the reverse inequality, equality follows.
\end{proof}
