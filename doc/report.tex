...
Without loss of generality, assume $f_1$ dominates (achieves the lower value) at the midpoint and therefore on $[m, r]$ where
\[
m = \frac{l+r}{2}.
\]
We designate $f_1$ as the \emph{representative} line for $[l, r]$, serving as the optimal answer for any query $x \in [m, r]$.
...
The appropriate child is determined by where the two candidates' relative ordering changes. Compare the two lines at the left endpoint $l$ and at the midpoint $m$: if the ordering differs (one line is lower at $l$ but the other is lower at $m$), the intersection lies in $[l, m]$, so the loser is routed to the left child; if the ordering is the same, the intersection lies in $[m, r]$, so the loser is routed right.
...
\textbf{Remark (tie-breaking).} When both lines achieve the same value at $m$, the strict $<$ comparison on line~8 keeps the resident line and pushes the incoming line down. This convention is arbitrary; since both lines achieve the same value at $m$, either choice satisfies the invariant.

\textbf{Remark (duplicate lines).} If the incoming line is identical to the resident line, no swap occurs and the duplicate is routed downward until it reaches maximum depth, where it is discarded. This is correct: inserting the same line twice does not change the lower envelope.
