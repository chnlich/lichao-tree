...
Since $L^*$ enters at the root (on $P$) and is not stored on $P$ in the final tree, at some node $w \in P$, $L^*$ was pushed to the child not containing $x_0$. The line $S$ kept at $w$ satisfies $S(m_w) \leq L^*(m_w)$. Since two lines intersect at most once, the intersection of $S$ and $L^*$ (if any) lies in the half where $L^*$ was pushed. Therefore $S$ dominates $L^*$ on the half containing $x_0$, and by Lemma~\ref{lem:linear-dom}, $S(x_0) \leq L^*(x_0)$, contradicting the assumption.
...
\item If $S$ is routed away from $x_0$: by the same reasoning, $C(x_0) \leq S(x_0) \leq L^*(x_0)$, and $C$ is at $w \in P$. If $C$ is also displaced, repeat.
\end{itemize}
