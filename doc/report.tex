...
Both operations must execute in logarithmic time to achieve acceptable performance for large-scale applications. The LICT achieves $O(\log C)$ per operation, where $C$ is the coordinate range.
...
Each node in the tree represents an interval $[l, r]$ and stores one line. During insertion, a line is \emph{routed} through the tree following a single path from root to leaf. At each node on this path, the new line is compared with the currently stored line. The line that performs better (yields lower value) at the midpoint $m = \lfloor(l+r)/2\rfloor$ is \emph{stored} at that node; the worse line continues downward to be routed through the appropriate child subtree.

\textbf{Key invariant.} A line stored at a node is the best among all lines routed through that node at the midpoint of the node's interval. This is \emph{local} optimality: the stored line minimizes $kx + m$ at $m$ among lines routed to that specific node, but may not be globally optimal at $m$ if other lines were routed down different branches.

\textbf{Why this maintains the lower envelope.} The LICT stores at each node the line that wins at the midpoint among lines routed there. Because two lines intersect at most once, if line $A$ beats line $B$ at the midpoint but loses at one endpoint, then $B$ can only win on the side of the midpoint containing that endpoint. Thus $B$ is routed to exactly one child---the side where it might win. At query time, we traverse the path to $x_0$ and take the minimum over all stored lines on that path. For any line $L$ and query point $x_0$, $L$ is stored at some node on the path to $x_0$ if and only if $L$ wins at that node's midpoint among lines routed there. Since the path covers all intervals containing $x_0$, the minimum among stored lines on the path equals the global lower envelope at $x_0$.

Insertion proceeds as follows: if the new line is better at the midpoint, the lines are swapped and the worse line continues down. The key geometric insight is that two distinct lines intersect at most once. Therefore, if a line is worse at the midpoint, it can be better than the stored line on at most one side of the midpoint, never both. This property guarantees that the worse line need only be propagated down a single child path, ensuring the algorithm terminates in $O(\log C)$ steps.
