...
% Root node [0, 8]
\node[draw, rectangle, fill=blue!10, minimum width=2.0cm] (root) at (0,0) {$[0, 8]$: $y=x$};
\node[below=0.1cm of root] {\small $m=4$};

% Level 1 nodes
\node[draw, rectangle, fill=blue!10, minimum width=2.0cm] (l1) at (-1.8,-2.5) {$[0, 4]$: $y=2x$};
\node[below=0.1cm of l1] {\small $m=2$};
\node[draw, rectangle, minimum width=2.0cm] (r1) at (1.8,-2.5) {$[4, 8]$: $y=x$};

% Level 2 nodes
\node[draw, rectangle, minimum width=2.0cm] (l2l) at (-2.8,-5) {$[0, 2]$: $-$};
\node[draw, rectangle, minimum width=2.0cm] (l2r) at (-0.8,-5) {$[2, 4]$: $-$};
\node[draw, rectangle, fill=red!10, minimum width=2.0cm] (r2l) at (0.8,-5) {$[4, 6]$: $y=10$};
\node[below=0.1cm of r2l] {\small $m=5$};
\node[draw, rectangle, minimum width=2.0cm] (r2r) at (2.8,-5) {$[6, 8]$: $-$};
...
\draw[thick, blue] (0,0) -- (8,8) node[right] {$y=x$ (root)};

% Line y = 2x (stored at [0,4], routed to left)
\draw[thick, green!60!black] (0,0) -- (4,8) node[above] {$y=2x$ ($[0,4]$)};

% New line y = 10 (inserted, stored at [4,6])
\draw[thick, red, dashed] (0,10) -- (8,10) node[right] {$y=10$ (new)};
...
% Annotations
\node[align=left, font=\scriptsize] at (4,-4.5) {\textbf{Step 1:} At root $[0,8]$: $y=2x$ lower at $m=4$?\\
No ($8 > 4$), so swap and route to right.\\
\textbf{Step 2:} At $[4,8]$: $y=10$ lower at $m=6$?\\
No ($10 > 6$), but check $[4,6]$: $y=10$ lower at $m=5$?\\
Yes ($10 > 5$), store here.};
\end{scope}
\end{tikzpicture}
...
% Level 1 nodes
\node[draw, rectangle, fill=green!10, minimum width=2.0cm] (l1) at (-1.8,-2.5) {$[0, 4]$: $y=2x$};
\node[below=0.1cm of l1] {\small $m=2$};
\node[draw, rectangle, minimum width=2.0cm] (r1) at (1.8,-2.5) {$[4, 8]$: $y=x$};

% Level 2 nodes
\node[draw, rectangle, fill=yellow!20, minimum width=2.0cm] (l2l) at (-2.8,-5) {$[0, 2]$: $-$};
\node[draw, rectangle, minimum width=2.0cm] (l2r) at (-0.8,-5) {$[2, 4]$: $-$};
\node[draw, rectangle, minimum width=2.0cm] (r2l) at (0.8,-5) {$[4, 6]$: $y=10$};
\node[draw, rectangle, minimum width=2.0cm] (r2r) at (2.8,-5) {$[6, 8]$: $-$};
...
\node[circle, draw, red, thick, minimum size=0.5cm] at (-2.8,-5) {};
\node[below=0.3cm of l2l, red, font=\scriptsize] {\small Query $x=1$};

% Legend
\node[align=left, font=\scriptsize] at (0,-6.5) {\textbf{Query path:} $[0,8] \to [0,4] \to [0,2]$\\
Lines evaluated: $y=x$, $y=2x$};
\end{scope}
...
% Query line at x=1
\draw[thick, red, dashed] (1,-0.5) -- (1,10);
\node[red, font=\scriptsize, rotate=90] at (1.3,8.5) {Query $x=1$};
...
\node[right, font=\scriptsize] at (1.2,1) {$y=x \to 1$};
\filldraw[green!60!black] (1,2) circle (2pt);
\node[right, font=\scriptsize] at (1.2,2.3) {$y=2x \to 2$};

% Minimum indicator
\draw[thick, ->, red] (2,0.3) -- (1.1,0.9);
\node[right, font=\scriptsize] at (2,0.3) {\textbf{Min:} $1$};

% Evaluation table
\node[draw, rectangle, fill=white, font=\scriptsize] at (4.5,-2.5) {
\begin{tabular}{l|c}
