...
\]
Since two distinct lines intersect at most once, one line must be lower than the other on at least half of the interval.

Without loss of generality, assume $f_1$ achieves the lower value at the midpoint $m = \frac{l+r}{2}$, and that the intersection of the two lines (if it exists within $[l, r]$) falls in the left half $[l, m]$. Then by Lemma~\ref{lem:linear-dom} (Linear Domination), $f_1$ dominates $f_2$ on the entire right half $[m, r]$. We designate $f_1$ as the \emph{representative} line for the node, serving as the locally optimal answer for queries in $[m, r]$.

The key insight is that $f_2$ only requires consideration when the query falls in the opposite half $[l, m]$, which comprises at most half the original interval. This observation enables a recursive decomposition: either the query is answered by the current representative, or we recurse on a subproblem of strictly half the size.
...
\item \textbf{All on Hull:} Lines $y = -(i+1)x + (i+1)^2$ for $i \in [0, N-1]$. All $N$ lines contribute to the hull, since the dual points $(-(i+1),\,(i+1)^2)$ lie on the convex parabola $b = k^2$, so all are vertices of the lower hull.
\end{itemize}
\end{itemize}
